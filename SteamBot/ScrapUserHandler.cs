...
namespace SteamBot
{
    ...
    {
        public static string BotVersion = "1.0.5";
        public static int SellPricePerKey = 31; // price in scrap, e.g. 31 / 9 = 3.55 ref
        public static int BuyPricePerKey = 29; // price in scrap, e.g. 29 / 9 = 3.33 ref
        ...
        public override void OnMessage(string message, EChatEntryType type)
        {
            ...
            {
                Bot.SteamFriends.SendChatMessage(OtherSID, type, "Here's my backpack: " + BackpackURL);
            }
            else if (message == "buy")
            {
            ...
            {
                Bot.SteamFriends.SendChatMessage(OtherSID, EChatEntryType.ChatMsg, HelpMsg);
            }
            // ADMIN commands
            ...
            else if (message == ".canceltrade")
            {
                ...
                {
                    Trade.CancelTrade();
                    Bot.SteamFriends.SendChatMessage(currentSID, EChatEntryType.ChatMsg, "An admin has forcefully cancelled the trade. Whatever you were doing, he probably wants you to stop.");
                }
            }
            else
            {
        ...
        public override void OnTradeInit()
        {
            ...
            Trade.SendMessage("Welcome to Rynoxxs public scrapbanking bot (v" + BotVersion + ")! This bot buys 2 weapons for 1 scrap or sells 2 weapons for 1 scrap. If you are looking to BUY weapons, type \"buy\", otherwise you can simply add your weapons and I will do the rest.");
            ...
            Trade.SendMessage("And if you want to donate items, type \"donate\".");
        }
        ...
        public override void OnTradeMessage(string message)
        {
            ...
            if (message == "buy")
            {
                ...
                Bot.SteamFriends.SendChatMessage(OtherSID, EChatEntryType.ChatMsg, "You have decided to buy weapons. Check my backpack at " + BackpackURL + " to see what I have! Remember the time limit, so make it quick!");
                buyWeapons = true;
            }
            else if (message == "help")
            {
                Trade.SendMessage(HelpMsg);
            }
            else if (message == "donate")
            {
                if (!IsDonating)
                {
                    IsDonating = true;
                    Trade.SendMessage("Thank you, just add items you want to donate! To cancel the donation just cancel this trade.");
                }
            }
            else if (message == "backpack")
            {
                Trade.SendMessage("Here's my backpack: " + BackpackURL);
            }
            else if (buyWeapons)
            {
        ...
        public override void OnTradeAccept()
        {
            if (Validate() || IsAdmin)
            {
                ...
                if (success)
                {
                    if (!IsDonating)
                    {
                        Bot.SteamFriends.SendChatMessage(OtherSID, EChatEntryType.ChatMsg, "Thanks for the trade!");
                    }
                    else
                    {
                        Bot.SteamFriends.SendChatMessage(OtherSID, EChatEntryType.ChatMsg, "Thanks for the donation!");
                    }
        ...
        public bool Validate()
        {
            ...
            else
            {
                if (botScrapAdded > 0)
                {
                    errors.Add("The bot has added scraps for a donation!");
                    Bot.log.Warn("The bot has added scraps for a donation!");
                }

                if (botWepAdded > 0)
                {
                    errors.Add("The bot has added weapons for a donation!");
                    Bot.log.Warn("The bot has added weapobs for a donation!");
                }
            }
        ...
        {
            Bot.SteamFriends.SendChatMessage(OtherSID, EChatEntryType.ChatMsg, "Hi. You have added Rynoxxs scrapbanking bot! I buy two weapons for one scrap and sell two weapons for one scrap. Check my backpack if you are buying weapons, and then trade me to begin!");
